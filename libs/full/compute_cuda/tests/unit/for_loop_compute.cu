    ...
    target_vector& d_C, std::vector<int> const& ref)
{
    ...
    hpx::for_loop_n(hpx::parallel::execution::par.on(exec), d_A.data(),
        d_A.size(), hpx::parallel::induction(d_B.data()),
        hpx::parallel::induction(d_C.data()),
        [] HPX_HOST_DEVICE(int* A, int* B, int* C) { *C = *A + 3.0 * *B; });

    std::vector<int> h_C(d_C.size());
    hpx::parallel::copy(
        hpx::parallel::execution::par, d_C.begin(), d_C.end(), h_C.begin());
...
int hpx_main(hpx::program_options::variables_map& vm)
{
    ...
    // copy data to device
    hpx::future<void> f = hpx::parallel::copy(
        hpx::parallel::execution::par(hpx::parallel::execution::task),
        h_A.begin(), h_A.end(), d_A.begin());

    hpx::parallel::copy(
        hpx::parallel::execution::par, h_B.begin(), h_B.end(), d_B.begin());
