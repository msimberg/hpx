...
void trim_all_test()
{
    ...
    HPX_TEST(
        trim_all_copy_if(
            std::string("123abc127deb456"),
            is_classified(std::ctype_base::digit) )=="abc1deb" );
    HPX_TEST( trim_all_copy_if( std::string("<>abc<>def<>"), is_any_of( "<<>>" ) )=="abc<def" );
}
...
void trim_fill_test()
{
    ...
    HPX_TEST( trim_fill_copy_if( std::string("<>abc<>def<>"), "-", is_any_of( "<<>>" ) )=="abc-def" );
}
