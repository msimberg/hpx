...
///////////////////////////////////////////////////////////////////////////////
BOOST_FUSION_ADAPT_STRUCT(hpx::threads::detail::spec_type,
...
namespace hpx { namespace threads { namespace detail {
    ...
    struct mappings_parser : qi::grammar<Iterator, mappings_type()>
    {
          ...
          : mappings_parser::base_type(start)
        {
            ...
            distribution = partlit("compact") >> qi::attr(compact) |
                partlit("scatter") >> qi::attr(scatter) |
                partlit("balanced") >> qi::attr(balanced) |
                partlit("numa-balanced") >> qi::attr(numa_balanced);

            thread_spec = partlit("thread") >> ':' >> specs >>
                qi::attr(spec_type::thread);

            pu_spec = socket_spec >> core_spec >> processing_unit_spec
                //                 |   '~' >> pu_spec
                ;

            socket_spec = partlit("socket") >> ':' >> specs >>
                    qi::attr(spec_type::socket) |
                partlit("numanode") >> ':' >> specs >>
                    qi::attr(spec_type::numanode) |
                qi::attr(spec_type::unknown);

            core_spec = -qi::lit('.') >> partlit("core") >> ':' >> specs >>
                    qi::attr(spec_type::core) |
                qi::attr(spec_type::unknown);

            processing_unit_spec = -qi::lit('.') >> partlit("pu") >> ':' >>
                    specs >> qi::attr(spec_type::pu) |
                qi::attr(spec_type::unknown);

            specs = spec % ',';

            spec = qi::uint_ >> -(qi::int_) |
        ...
        topology const& t, spec_type const& s, error_code& ec)
    {
        ...
        default:
            HPX_THROWS_IF(ec, bad_parameter, "extract_socket_or_numanode_mask",
                hpx::util::format("unexpected specification type {}",
        ...
        bounds_type const& b, error_code& ec)
    {
        ...
        {
            HPX_THROWS_IF(ec, bad_parameter, "decode_mapping",
                hpx::util::format("no {1} mapping bounds are specified",
            ...
            for (std::size_t num_core = 0; num_core < num_cores; ++num_core)
            {
                if (any(affinities[num_thread]))
                {
                    HPX_THROWS_IF(ec, bad_parameter,
                        ...
                        hpx::util::format("affinity mask for thread {1} has "
                                          "already been set",
                            num_thread));
                    return;
                }
